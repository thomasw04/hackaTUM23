<template>
	<l-map :use-global-leaflet="false" v-model="zoomLvl" v-model:zoom="zoomLvl" :center="searchPLZCoords">
		...
		<template v-if="searchPLZCoords">
			<l-circle-marker v-if="searchRadius" :lat-lng="searchPLZCoords" :radius="searchRadius" color="rgba(255,0,0,0.3)" />
			<l-marker :lat-lng="searchPLZCoords" />
		</template>

		<l-marker v-for="sp in serviceProviders" :key="sp.id" :lat-lng="getNormalCoords(sp)" :title="sp.last_name" :alt="sp.last_name">
			<l-popup>
				<h3>{{ sp.first_name }} {{ sp.last_name }}</h3>
				<p class="always-light" style="margin: 0"><b>{{ sp.city }}</b></p>
