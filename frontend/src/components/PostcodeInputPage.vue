  ...
  <div class="search-page">
    ...
    <div class="field pt-2">
      <form id="search-form" action="/search" @submit.prevent>
        <div id="search-container" class="custom-outline control has-icons-left has-icons-right is-large" :class="{ 'is-loading': isLoadingAutocomplete }">
          <input autofocus id="search-bar" v-model="searchQuery" class="input is-large" type="text" placeholder="Postal code" autocomplete="off" @input="loadAutocomplete" @keydown.down.prevent="handleArrowDown" @keydown.up.prevent="handleArrowUp" @keydown.enter="handleEnter(false)" />
        ...
        <div class="custom-outline pt-2">
          <button style="background-color: #0271c2" class="button is-primary is-fullwidth" @click="handleEnter(true)">Search</button>
        </div>
      </form>
    </div>
  </div>
